  ...
  return (
    <div className="relative min-h-[54vh] bg-white flex items-center px-[6%] pt-32 pb-20 mt-[106px] overflow-hidden"
      ...
      <div className="relative z-[2] max-w-[1240px] w-full">
        ...
        <p className="text-[1.05rem] leading-[1.82] text-body max-w-[580px] mb-8">{sub}</p>
        ...
        <div className="flex flex-wrap gap-4 mb-10">
          {[
            ...
            { label: '14+ Years Track Record', icon: '✓' },
          ].map((item, i) => (
            <div key={i} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/60 border border-[rgba(45,43,107,.15)] shadow-sm hover:shadow-md hover:bg-white transition-all duration-200">
              <span className="text-[.9rem]">{item.icon}</span>
              <span className="text-[.82rem] font-medium text-dark">{item.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <Link
            to="/contact"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-[.9rem] font-bold text-dark bg-gg
                       shadow-[0_4px_16px_rgba(201,168,76,.3)] relative overflow-hidden transition-all duration-[260ms]
                       hover:-translate-y-[2px] hover:shadow-[0_8px_24px_rgba(201,168,76,.48)]
          ...
          <button className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-[.9rem] font-semibold
                             text-b4 border-[1.5px] border-[rgba(45,43,107,.2)] bg-white/40 hover:bg-white transition-all duration-200">
            Learn More
          </button>
        </div>
      </div>
                ...
                {[
                  ...
                  return (
                    <div key={i} className="bg-pale border-[1.5px] border-border rounded-[13px] p-4 transition-all duration-[250ms] hover:border-b4 hover:bg-pale2 hover:-translate-y-[3px]">
                      <div className="w-6 h-6 text-b4 mb-[7px]">
      ...
      <section className="bg-page py-24 px-[6%]">
        <div className="max-w-[1240px] mx-auto">
          <div className="text-center mb-16">
            <SLabel center>Our Approach</SLabel>
            <SH2 center>How We <em className="not-italic grad-text">Work</em> With You</SH2>
            <p className="text-[.97rem] leading-[1.8] text-muted max-w-[560px] mx-auto">
            ...
            {[
              {
                ...
                icon: Eye,
                title: 'Understand',
                desc: 'We deep-dive into your business, challenges, and goals to build a complete understanding before writing a single line of code.'
              },
              {
                num: '02',
                icon: Lightbulb,
                title: 'Strategize',
                desc: 'Our architects design scalable solutions tailored to your needs, considering security, performance, and future growth.'
              },
              {
                num: '03',
                icon: Code2,
                title: 'Build',
                desc: 'Agile development with regular sprints, continuous integration, and quality assurance at every step.'
              },
              {
                num: '04',
                icon: Rocket,
                title: 'Launch & Support',
                desc: 'Seamless deployment, training, and 24/7 support to ensure your solution succeeds from day one.'
              },
            ].map((step, i) => {
              const IconComponent = step.icon;
              return (
                <div key={i} className={`sr d${i + 1} group relative`}>
            ...
            {values.map((v, i) => {
              ...
              return (
                <div key={i} className={`sr d${(i % 3) + 1} group relative overflow-hidden bg-page border-[1.5px] border-border rounded-[20px] p-[30px] transition-all duration-300 hover:-translate-y-[7px] hover:shadow-md hover:border-[rgba(45,43,107,.12)] before:content-[''] before:absolute before:top-0 before:left-0 before:right-0 before:h-[3px] before:bg-gg before:scale-x-0 before:origin-left before:transition-transform before:duration-[350ms] hover:before:scale-x-100`}>
                  <div className="w-[52px] h-[52px] rounded-[14px] bg-pale border-[1.5px] border-border flex items-center justify-center text-b4 mb-[17px] transition-all duration-300 group-hover:bg-gm group-hover:border-transparent group-hover:text-white">
      ...
      <section className="bg-page py-24 px-[6%]">
        <div className="max-w-[1240px] mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            {/* Left — Icon & Statement */}
            <div className="sr-l">
              <div className="w-20 h-20 rounded-2xl bg-gm flex items-center justify-center text-white mb-8 shadow-lg">
                <Lock size={40} strokeWidth={1.5} />
              </div>
              <SLabel>Your Trust, Our Priority</SLabel>
              <SH2>Your Ideas Are <em className="not-italic grad-text">Safe</em> With Us</SH2>
              <p className="text-[1rem] leading-[1.82] text-body mb-6">
              ...
              <p className="text-[.95rem] leading-[1.75] text-muted mb-8">
            ...
            {/* Right — Trust Cards */}
            <div className="sr-r space-y-5">
              {[
                {
                  icon: Shield,
                  title: 'Complete Confidentiality',
                  desc: 'Every project is under strict NDA. Your ideas, code, and strategy remain yours and yours alone.'
                },
                {
                  icon: Lock,
                  title: 'Data Security',
                  desc: 'Multi-layer encryption, secure servers, and compliance with GDPR, HIPAA, and industry standards.'
                },
                {
                  icon: Users,
                  title: 'Professional Integrity',
                  desc: 'Our team is vetted, trained, and committed to ethical practices. Your trust is everything to us.'
                },
                {
                  icon: CheckCircle,
                  title: 'Proven Track Record',
                  desc: '14+ years of zero security breaches. 300+ enterprises trust us with their most sensitive projects.'
                },
              ].map((item, i) => {
                const IconComponent = item.icon;
                return (
                  <div key={i} className="group bg-white border-[1.5px] border-border rounded-[16px] p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-b4">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-[10px] bg-pale border-[1.5px] border-border flex items-center justify-center text-b4 flex-shrink-0 transition-all duration-300 group-hover:bg-gm group-hover:border-transparent group-hover:text-white">
                        <IconComponent size={20} strokeWidth={1.8} />
                      </div>
